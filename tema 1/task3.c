...
int main() {
    ...
    unsigned short operanzi_short[MAX], numar = 0, operatie[MAX] = {0}, rez = 0;
    ...
    if (nr_printrest != nr_print)
        nr_print++;
    // nr_print = cate numere voi citii
    for (i = 0; i < nr_print; i++) {
        scanf("%hu", & numar);
        j = lastbit;
        while (j >= 0) {
            if (j - Dim < -1) {
                operanzi_short[opN] = cbin(j + 1, numar, j);
                scanf("%hu", & numar);
                i++;
                rez = cbin(Dim - j - 1, numar, lastbit);
                operanzi_short[opN] = (operanzi_short[opN] << (Dim - j - 1)) | rez;
                opN++;
                j = lastbit - (Dim - j) + 1;
            }
            // cazul in care dim e un nr ciudat
            else {
    ...
    while (i < nr_opi) {
        ...
        // fac calculele
            ...
            i++;
        // trec la urmatoarea instructiune
    }
    printf("%d\n", operanzi[0]);
    return 0;
}